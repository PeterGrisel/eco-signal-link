...
async function loadContext(supabase: any) {
  ...
  const siteUrl = cfg.site_url || Deno.env.get("PUBLIC_SITE_URL") || "https://example.com";
  const siteName = cfg.name || Deno.env.get("SITE_NAME") || "Your Site";
  // Topic focus for the glossary. Configure per-site via seo_settings.config.glossary_focus,
  // else fall back to the blog theme, else a generic instruction.
  const glossaryFocus = cfg.glossary_focus || cfg.blog_theme || "your subject area";

  // Optional curated tool library (see generate-article). Off unless enabled.
  const toolTable = cfg.tool_library_enabled ? Deno.env.get("TOOL_LIBRARY_TABLE") : null;

  const [{ data: tools }, { data: posts }, { data: existing }] = await Promise.all([
    toolTable
      ? supabase.from(toolTable).select("name, category").eq("published", true).limit(80)
  ...
  // Fixed pages the writer may link to. Configure via seo_settings.config.fixed_pages = [{title,path}].
  const fixedPages: Array<{ title: string; path: string }> =
    Array.isArray(cfg.fixed_pages) && cfg.fixed_pages.length
      ? cfg.fixed_pages
      : [{ title: "Home", path: "/" }, { title: "Blog", path: "/blog" }];

  const internalLinks = [
    ...fixedPages.map((p) => `- ${p.title}: ${siteUrl}${p.path}`),
    ...(existing || []).slice(0, 12).map((t: any) => `- ${t.term}: ${siteUrl}/woordenboek/${t.slug}`),
  ].join("\n");

  return { siteUrl, siteName, glossaryFocus, toolsContext, blogContext, existingTerms, existingTermsLower, internalLinks };
}
...
async function proposeTerms(LOVABLE_API_KEY: string, ctx: any, count: number): Promise<string[]> {
  const res = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: "system", content: `Je stelt ${count} nieuwe termen voor het woordenboek van ${ctx.siteName} voor. Focus: ${ctx.glossaryFocus}. Geen tool- of merknamen. Mix Nederlands en gangbare Engelse vaktermen waar die gebruikelijk zijn. Antwoord met alleen een lijst, één term per regel, geen nummering, geen uitleg.` },
...
async function generateOne(
  ...
  try {
    ...
    const toolsBlock = ctx.toolsContext
      ? `\nCONTEXT (curated tool library — link, don't promote):\n${ctx.toolsContext}\n`
      : "";
    const systemPrompt = `Je bent een redacteur bij ${ctx.siteName}. Je schrijft glasheldere woordenboek-definities. Focus: ${ctx.glossaryFocus}.
...
- "## Waarom het ertoe doet" — concrete waarde voor de lezer.
- "## Hoe je het inzet" — 3-5 bullets met praktische toepassing.
- Sluit af met 1 zin die linkt naar een relevante interne pagina uit de lijst hierboven.`;
