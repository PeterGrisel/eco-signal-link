...
function buildSvg(opts: {
  ...
  const brandName = Deno.env.get("SITE_NAME") || "Brand";
  const rawTitle = `${company} × ${brandName}`;
...
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630">
  ...
  <text x="80" y="560" font-family="Inter" font-size="22" font-weight="700" fill="#FFFFFF" opacity="0.7">${escapeXml((Deno.env.get("PUBLIC_SITE_URL") || "https://example.com").replace(/^https?:\/\//, ""))}</text>
</svg>`;
}
    ...
    if (data.logo_url) {
      try {
        const logoAbs = data.logo_url.startsWith("http")
          ? data.logo_url
          : `${Deno.env.get("PUBLIC_SITE_URL") || "https://example.com"}${data.logo_url}`;
