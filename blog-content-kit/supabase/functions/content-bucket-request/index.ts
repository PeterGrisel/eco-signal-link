...
const SITE_URL = Deno.env.get("PUBLIC_SITE_URL") || "https://example.com";
const SITE_NAME = Deno.env.get("SITE_NAME") || "Your Site";
// Verified sender for your transactional email provider, e.g. "Your Site <hi@notify.yoursite.com>"
const MAIL_FROM = Deno.env.get("MAIL_FROM") || `${SITE_NAME} <no-reply@example.com>`;
  ...
  try {
    ...
    const html = `<!DOCTYPE html><html><body style="font-family:Inter,Arial,sans-serif;background:#fff;color:#121212;padding:24px;">
      <div style="max-width:520px;margin:0 auto;border:1px solid #eee;border-radius:12px;padding:32px;">
        <div style="font-family:'Space Grotesk',Arial,sans-serif;font-weight:700;font-size:18px;">${SITE_NAME}</div>
