...
serve(async (req) => {
  ...
  try {
    ...
    const siteUrl: string = config.site_url || Deno.env.get("PUBLIC_SITE_URL") || "https://example.com";
    const siteName: string = config.name || Deno.env.get("SITE_NAME") || "de site";
    const topicFocus: string = config.blog_theme || "the site's core topics";
    ...
    // 2. Ask AI
    const systemPrompt = `Je bent een SEO content strateeg voor ${siteName}.
Je taak: één long-tail SEO pagina voorstellen die een concrete gap dicht t.o.v. concurrenten.

REGELS:
- Long-tail: zoekvolume <300/mnd, lage concurrentie (SEO Avalanche strategie).
- B1 taalniveau, "u/uw", max 12 woorden per zin, geen em-dashes.
- Sentence case in headers (alleen eerste woord/eigennaam hoofdletter).
- Vendor-agnostisch: GEEN tool-namen als productbelofte (mogen genoemd worden als voorbeeld, niet als afhankelijkheid).
- Onderwerp moet aansluiten bij het thema van de site: ${topicFocus}
