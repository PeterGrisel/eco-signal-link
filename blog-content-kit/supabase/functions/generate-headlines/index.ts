...
serve(async (req) => {
  ...
  try {
    ...
    const niche = settings.summary || "your niche";
    const audience = settings.target_audience_summary || "your target audience";
    const blogTheme = settings.blog_theme || "practical, useful content for your audience";
    ...
    if (gscData?.length) {
      // Brand terms to exclude from the non-branded traffic calc. Derived from the
      // site name; add extra variants per-site via seo_settings.config.brand_terms (string[]).
      const brandTerms = [
        siteName.toLowerCase(),
        ...((settings.brand_terms as string[] | undefined) || []).map((t) => t.toLowerCase()),
      ];
