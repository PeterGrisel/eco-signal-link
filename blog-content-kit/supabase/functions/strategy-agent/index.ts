...
async function fetchPageContent(url: string): Promise<string> {
  try {
    const res = await fetch(url, { headers: { "User-Agent": (Deno.env.get("SITE_NAME") || "Site") + "-StrategyAgent/1.0" } });
    if (!res.ok) return "";
...
serve(async (req) => {
  ...
  try {
    ...
    const audience = settings.target_audience_summary || "your target audience";
    ...
    if (gscData?.length) {
      ...
      const brandTerms = [siteName.toLowerCase(), ...((settings.brand_terms as string[] | undefined) || []).map((t: string) => t.toLowerCase())];
...
Structuur het rapport als een duidelijke ${siteName} VS Concurrent vergelijking:
