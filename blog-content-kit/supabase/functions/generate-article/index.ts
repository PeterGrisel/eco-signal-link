...
// --- OPTIONAL: curated tool/resource library injected into articles ---
// Disabled by default. Enable per-site by setting seo_settings.config.tool_library_enabled = true
// and TOOL_LIBRARY_TABLE to the name of a table with columns:
//   name, category, description, website, link_status, published, sort_order
// This is generic: it was "de Groeistack" in the origin project, but any curated
// link/resource table works. Leave it off and the engine simply skips this block.
async function loadToolLibrary(settings: Record<string, unknown>) {
  if (!settings?.tool_library_enabled) return [];
  const table = Deno.env.get("TOOL_LIBRARY_TABLE");
  if (!table) return [];
  const supabase = createClient(
    Deno.env.get("SUPABASE_URL")!,
    Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!
  ...
  // only use entries with a working link
  return (data || []).filter(
...
function buildToolLibraryContext(
  ...
  for (const [category, items] of grouped) {
    ...
    const lines = items
      .slice(0, 8) // cap per category so context doesn't explode
      .map(
        (t) =>
  ...
  return `\nTOOL LIBRARY (our curated resource library - use only where relevant):
${blocks.join("\n\n")}

TOOL LIBRARY USAGE:
- Reference entries from this list when the section topic overlaps their category
- ALWAYS use the exact markdown link from the list above: [Name](website)
- Never invent URLs and never use tools that are not in this list
- Stay vendor-neutral: name 2-3 alternatives side by side, do not push any single vendor
- Weave them naturally into the text, not as a separate list
- These tool links count as external links for the SEO check`;
}
...
serve(async (req) => {
  ...
  try {
    ...
    const [existingPosts, toolLibrary] = await Promise.all([
      loadExistingPosts(),
      loadToolLibrary(settings),
    ]);
    ...
    // All brand/site identity comes from seo_settings.config (edited in the admin UI),
    // with env fallbacks so a fresh clone still runs before settings are filled in.
    const siteName = settings.name || Deno.env.get("SITE_NAME") || "Your Site";
    const lang = settings.primary_language || "Nederlands";
    const targetAudience = audience || settings.target_audience_summary || "your target audience";
    ...
    const siteUrl = settings.site_url || Deno.env.get("PUBLIC_SITE_URL") || "https://example.com";
    // Fixed site pages the writer may link to internally. Configure per-site in
    // seo_settings.config.fixed_pages = [{ title, path }]. Defaults to just Home + Blog.
    const fixedPages: Array<{ title: string; path: string }> =
      Array.isArray(settings.fixed_pages) && settings.fixed_pages.length
        ? settings.fixed_pages
        : [
            { title: "Home", path: "/" },
            { title: "Blog", path: "/blog" },
          ];
    // Optional category labels for the tool library, configurable per-site.
    const toolCategoryLabels: Record<string, string> =
      (settings.tool_library_category_labels as Record<string, string>) || {};
...
Vaste pagina's:
${fixedPages.map((p) => `- [${p.title}](${siteUrl}${p.path})`).join("\n")}`
      : "";

    const toolLibraryContext = buildToolLibraryContext(toolLibrary, toolCategoryLabels);

    const systemPrompt = `Je bent een expert SEO content schrijver voor ${siteName}. Je schrijft in het ${lang} voor de volgende doelgroep: ${targetAudience}.
