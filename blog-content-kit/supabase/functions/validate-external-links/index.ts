...
// Internal hosts are skipped during external-link validation. Derived from
// PUBLIC_SITE_URL, plus any extra hosts in INTERNAL_HOSTS_EXTRA (comma-separated,
// e.g. your Lovable preview domain).
const INTERNAL_HOSTS = (() => {
  ...
  for (const extra of (Deno.env.get("INTERNAL_HOSTS_EXTRA") || "").split(",")) {
    const t = extra.trim();
    if (t) hosts.add(t);
  }
  return hosts;
